...
IObject *ParserClass::newValue(Allocator *allocator)
{
    ...
    obj->setSlotValue("value_stack", allocator->newArray(10));
    return obj;
}
...
Value *ParserClass::dispatch(int id, QVector<Value *>args)
{
    ...
    case 0: // ادفع.مسار.بديل
        ...
        parser->stack.push(ParseFrame(*args[1]->unboxStr(), parser->pos));
        return NULL;
    ...
    case 1: // اذهب.مسار.بديل
        ...
        return allocator->newString(new QString(f.continuationLabel));
    ...
    case 6:    // انظر
        vi = parser->data->unboxIndexable();
        return vi->get(allocator->newInt(parser->pos));
    case 7:    // انظر.عديد
    ...
    case 8:    // يطل.على
        ...
        return allocator->newBool(str.mid(parser->pos,1) == *(args[1]->unboxStr()));
    ...
    case 10:     // تفرع
        rw->typeCheck(args[1], BuiltInTypes::StringType);
        rw->typeCheck(args[2], BuiltInTypes::StringType);
        parser->stack.push(ParseFrame(*args[2]->unboxStr()));
        return args[1];
    case 11:    // عد
        return allocator->newString(new QString(parser->stack.pop().continuationLabel));
    ...
    case 16:// تذكر
        ...
        str = *args[1]->unboxStr();
        if(!parser->memoize.contains(str))
        {
            parser->memoize[str] = QMap<int, ParseResult>();
        }
        parser->memoize[str][args[2]->unboxInt()] =
    ...
    case 17: // استرد.ذكرى
        ...
        rw->typeCheck(args[2], BuiltInTypes::IntType);
        str = *args[1]->unboxStr();
        pr = parser->memoize[str][args[2]->unboxInt()];
    ...
    case 18:  // هل.تذكر
        ...
        if(!parser->memoize.contains(str))
            return allocator->newBool(false);
        return allocator->newBool(parser->memoize[str].contains(args[2]->unboxInt()));
    }
}
